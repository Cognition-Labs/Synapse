...
function generateActivationDescription(checkName) {
  return (
    `activate the '${checkName}' check and ` +
...
function generateCheckOptions(checkParams) {
  ...
  program
    .option(`--no-${checkOption}`, generateDeactivationDescription(checkName), {
      isDefault: true,
    })
    .option(`--${checkOption}`, generateActivationDescription(checkName), {
      isDefault: true,
    })
...
if (!checksModule) {
  ...
  program
    .option('--eprime', generateActivationDescription('E-Prime'))
    .option(
      '--yes-eprime',
  ...
  // set custom ops, for example to lint a non-English document
  try {
...
if (hasChecks) {
  ...
  args.slice(1).forEach((arg) => {
    if (
      ...
      return
    const isValid = Object.keys(opts.checks).some(
      (check) => arg === `--${check}` || arg === `--no-${check}`
    )
    if (!isValid) {
  ...
  optionArgs.forEach((arg) => {
    ...
    const isValid = program.options.some(
      (option) => arg === option.long || arg === option.short
    )
    if (!isValid) {
      handleInvalidArgument(arg)
    }
...
Object.keys(opts).forEach((name) => {
  if (
    typeof opts[name] !== 'boolean' &&
    name !== 'text' && // --text="text to check"
    name !== 'checks'
  ) {
    // --checks="custom-check-module"
    opts[name] = include
  }
})
...
function displaySuggestions(source, contents, options) {
  ...
  if (shouldParse) {
    ...
    console.log(
      annotate(contents, suggestions, true)
        .map((ann) =>
          [source, ann.line, ann.col, ann.reason.replace('\n', ' ')].join(':')
        )
        .join('\n')
    )
  } else {
