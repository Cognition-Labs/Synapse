import weaselWords from './lib/weasel'
import passiveVoice from './lib/passive'
import adverbWhere from './lib/adverbs'
import tooWordy from './lib/too-wordy'
import noCliches from './lib/cliches'
import ePrime from './lib/e-prime'
import lexicalIllusions from './lib/lexical-illusions'
import startsWithSo from './lib/starts-with-so'
import thereIs from './lib/there-is'
...
function dedup(suggestions) {
  ...
  return suggestions.reduce((memo, suggestion) => {
    ...
    if (!dupsHash[key]) {
      ...
      dupsHash[key].reason += ` and ${suggestion.reason.substring(
        suggestion.offset + 3
      )}`
    }
    return memo
